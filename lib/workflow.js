...
function parseArgs(prompt) {
  ...
  return {
    ...
    variantTag: promptHash(cleanPrompt).toString(16).slice(0, 8),
...
function resolveTemplate(inputTemplate, prompt) {
  ...
  // For generic prompts, deterministically diversify graph variants.
  const bucket = promptHash(text) % 3;
  if (bucket === 0) return "txt2img_fast";
  if (bucket === 1) return "txt2img_refine";
  return "txt2img_upscale";
}
...
function txt2imgWorkflow(parsed) {
  return {
    "1": {
      inputs: { ckpt_name: parsed.ckptName || DEFAULT_CKPT_NAME },
    ...
    "7": {
      inputs: { filename_prefix: `ComfyUI_Txt2Img_${parsed.variantTag}`, images: ["6", 0] },
...
function txt2imgRefineWorkflow(parsed) {
  return {
    ...
    "10": {
      inputs: { filename_prefix: `ComfyUI_Txt2Img_Refine_${parsed.variantTag}`, images: ["9", 0] },
      class_type: "SaveImage",
      _meta: { title: "Save Image" }
    }
  };
...
function txt2imgUpscaleWorkflow(parsed) {
  return {
    ...
    "9": {
      inputs: { filename_prefix: `ComfyUI_Txt2Img_Upscale_${parsed.variantTag}`, images: ["8", 0] },
      class_type: "SaveImage",
      _meta: { title: "Save Image" }
    }
  };
...
function img2imgWorkflow(parsed) {
  return {
    ...
    "8": {
      inputs: { filename_prefix: `ComfyUI_Img2Img_${parsed.variantTag}`, images: ["7", 0] },
      class_type: "SaveImage",
      _meta: { title: "Save Image" }
    }
  };
...
function upscaleWorkflow(parsed) {
  return {
    ...
    "5": {
      inputs: { filename_prefix: `ComfyUI_Upscale_${parsed.variantTag}`, images: ["4", 0] },
      class_type: "SaveImage",
      _meta: { title: "Save Image" }
    }
  };
